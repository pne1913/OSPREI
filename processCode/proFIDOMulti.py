...
# |------------------------------------------------------------|
def makeISplot(ResArr, ResArr2, dObj, dObj2, DoY, DoY2, SWpadF=12, SWpadB = 15, HiLite=None, plotn=False, tightDates=False, setTrange=False, useObsLim=False, satID=0, silent=True, satNames=[''], hasObs=False, ObsData=[None], BFs=[None], satCols=None):
    ...
    for key in ResArr.keys():
        ...
        if ResArr[key].FIDOtimes[satID] is not None: 
            ...
            if OSP.noDate:
                ...
                dates2 = ResArr2[key].FIDOtimes[satID]
            # |------------- Convert FIDO time to real dats --------------|    
            else:
                base = datetime.datetime(yr, 1, 1, 0, 0)
                base2 = datetime.datetime(yr2, 1, 1, 0, 0)
                if not OSP.doANT:
                    dates = np.array([base + datetime.timedelta(days=(i-1)) for i in ResArr[key].FIDOtimes[satID]])
                    dates2 = np.array([base2 + datetime.timedelta(days=(i-1)) for i in ResArr2[key].FIDOtimes[satID]])
                else:
                    dates = np.array([base + datetime.timedelta(days=(i+DoY)) for i in ResArr[key].FIDOtimes[satID]])
                    dates2 = np.array([base2 + datetime.timedelta(days=(i+DoY2)) for i in ResArr2[key].FIDOtimes[satID]])
                    
            # |------------- Plot the flux rope --------------|
            if not ResArr[key].sheathOnly[satID]:
                nowIdx = ResArr[key].FIDO_FRidx[satID]
                nowIdx2 = ResArr2[key].FIDO_FRidx[satID]
                #if lab[0]:
                    #if multiBF:
                        #for iii in BFidxs:
                            #axes[0].plot(dates[nowIdx], ResArr[key].FIDOBs[satID][nowIdx]+ResArr2[key].FIDOBs[satID][nowIdx2], linewidth=lw, color=co, zorder=zord, label=satNames[iii])
                    #else:
                        #axes[0].plot(dates[nowIdx], ResArr[key].FIDOBs[satID][nowIdx]+ResArr2[key].FIDOBs[satID][nowIdx2], linewidth=lw, color=co, zorder=zord, label=lab)
                            
                #else:
                    #axes[0].plot(dates[nowIdx], ResArr[key].FIDOBs[satID][nowIdx]+ResArr2[key].FIDOBs[satID][nowIdx2], linewidth=lw, color=co, zorder=zord)
                # axes[1].plot(dates[nowIdx], ResArr[key].FIDOBxs[satID][nowIdx]+ResArr2[key].FIDOBxs[satID][nowIdx2], linewidth=lw, color='blue', zorder=zord)
                # axes[2].plot(dates[nowIdx], ResArr[key].FIDOBys[satID][nowIdx]+ResArr2[key].FIDOBys[satID][nowIdx2], linewidth=lw, color=co, zorder=zord)
                # axes[3].plot(dates[nowIdx], ResArr[key].FIDOBzs[satID][nowIdx]+ResArr2[key].FIDOBzs[satID][nowIdx2], linewidth=lw, color=co, zorder=zord)
                # axes[4].plot(dates[nowIdx], ResArr[key].FIDOvs[satID][nowIdx]+ResArr2[key].FIDOvs[satID][nowIdx2], linewidth=lw, color=co, zorder=zord)
                # axes[5].plot(dates[nowIdx], (ResArr[key].FIDOtems[satID][nowIdx]+ResArr2[key].FIDOtems[satID][nowIdx2])/1e6, linewidth=lw, color=co, zorder=zord)
                # # Option to plot either n or Kp
                # if OSP.isSat or plotn:
                #     axes[6].plot(dates[nowIdx], ResArr[key].FIDOns[satID][nowIdx] + ResArr2[key].FIDOns[satID][nowIdx2], linewidth=lw, color=co, zorder=zord)
                # else:
                #     axes[6].plot(dates[nowIdx], ResArr[key].FIDOKps[satID][nowIdx] + ResArr2[key].FIDOKps[satID][nowIdx2], linewidth=lw, color=co, zorder=zord)
                #
                # |------------- Establish min/max date if not set --------------|    
                if mindate is None: 
                    mindate = dates[nowIdx[0]]
                    maxdate = dates[nowIdx[-1]]
                print("date1: ", mindate, maxdate)

                # if mindate2 is None:
                #     mindate2 = dates2[nowIdx[0]]
                #     maxdate2 = dates2[nowIdx[-1]]
                # print("date2: ", mindate2, maxdate2)

                print("date length:" ,len(dates),len(dates2))
                #if mindate < mindate2:
            ...
            #matches = merge(dates, dates2)
            #print("matches:", len(matches))



            axes[0].plot(dates, ResArr[key].FIDOBs[satID], linewidth=lw, color='darkred', zorder=zord)
            axes[0].plot(dates2, ResArr2[key].FIDOBs[satID], linewidth=lw, color='darkblue', zorder=zord)
            axes[1].plot(dates, ResArr[key].FIDOBxs[satID], linewidth=lw, color='darkred', zorder=zord)
            axes[1].plot(dates2, ResArr2[key].FIDOBxs[satID], linewidth=lw, color='darkblue', zorder=zord)
            axes[2].plot(dates, ResArr[key].FIDOBys[satID], linewidth=lw, color='darkred', zorder=zord)
            axes[2].plot(dates2, ResArr2[key].FIDOBys[satID], linewidth=lw, color='darkblue', zorder=zord)
            axes[3].plot(dates, ResArr[key].FIDOBzs[satID], linewidth=lw, color='darkred', zorder=zord)
            axes[3].plot(dates2, ResArr2[key].FIDOBzs[satID], linewidth=lw, color='darkblue', zorder=zord)
            axes[4].plot(dates, ResArr[key].FIDOvs[satID], linewidth=lw, color='darkred', zorder=zord)
            axes[4].plot(dates2, ResArr2[key].FIDOvs[satID], linewidth=lw, color='darkblue', zorder=zord)
            axes[5].plot(dates, ResArr[key].FIDOtems[satID], linewidth=lw, color='darkred', zorder=zord)
            axes[5].plot(dates2, ResArr2[key].FIDOtems[satID], linewidth=lw, color='darkblue', zorder=zord)
            axes[6].plot(dates, ResArr[key].FIDOns[satID], linewidth=lw, color='darkred', zorder=zord)
            axes[6].plot(dates2, ResArr2[key].FIDOns[satID], linewidth=lw, color='darkblue', zorder=zord)
            ...
            if len(ResArr[key].FIDO_shidx[satID]) != 0:
                ...
                nowIdx2 = ResArr2[key].FIDO_shidx[satID]
                if not ResArr[key].sheathOnly[satID]:
                    nowIdx = np.append(nowIdx, ResArr[key].FIDO_FRidx[satID][0])
                    #nowIdx2 = np.append(nowIdx2, ResArr2[key].FIDO_FRidx[satID][0])
                #axes[0].plot(dates[nowIdx], ResArr[key].FIDOBs[satID][nowIdx] + ResArr2[key].FIDOBs[satID][nowIdx2], '--', linewidth=lw, color=co, zorder=zord)
                #axes[1].plot(dates[nowIdx], ResArr[key].FIDOBxs[satID][nowIdx] + ResArr2[key].FIDOBxs[satID][nowIdx2], '--', linewidth=lw, color=co, zorder=zord)
                #axes[2].plot(dates[nowIdx], ResArr[key].FIDOBys[satID][nowIdx] + ResArr2[key].FIDOBys[satID][nowIdx2], '--', linewidth=lw, color=co, zorder=zord)
                #axes[3].plot(dates[nowIdx], ResArr[key].FIDOBzs[satID][nowIdx] + ResArr2[key].FIDOBzs[satID][nowIdx2], '--', linewidth=lw, color=co, zorder=zord)
                #axes[4].plot(dates[nowIdx], ResArr[key].FIDOvs[satID][nowIdx] + ResArr2[key].FIDOvs[satID][nowIdx2], '--', linewidth=lw, color=co, zorder=zord)
                #axes[5].plot(dates[nowIdx], (ResArr[key].FIDOtems[satID][nowIdx] + ResArr2[key].FIDOtems[satID][nowIdx2])/1e6, '--', linewidth=lw, color=co, zorder=zord)
            ...
            if len(ResArr[key].FIDO_SWidx[satID]) > 0:
                ...
                #     #axes[0].plot(dates[nowIdx], ResArr[key].FIDOBs[satID][nowIdx] + ResArr2[key].FIDOBs[satID][nowIdx2], ':', linewidth=lw, color=co, zorder=zord)
                #     axes[1].plot(dates[nowIdx], ResArr[key].FIDOBxs[satID][nowIdx] + ResArr2[key].FIDOBxs[satID][nowIdx], ':', linewidth=lw, color='red', zorder=zord)
                #     axes[2].plot(dates[nowIdx], ResArr[key].FIDOBys[satID][nowIdx] + ResArr2[key].FIDOBys[satID][nowIdx], ':', linewidth=lw, color=co, zorder=zord)
                #     axes[3].plot(dates[nowIdx], ResArr[key].FIDOBzs[satID][nowIdx] + ResArr2[key].FIDOBzs[satID][nowIdx], ':', linewidth=lw, color=co, zorder=zord)
                #     axes[4].plot(dates[nowIdx], ResArr[key].FIDOvs[satID][nowIdx] + ResArr2[key].FIDOvs[satID][nowIdx], ':', linewidth=lw, color=co, zorder=zord)
                #     axes[5].plot(dates[nowIdx], (ResArr[key].FIDOtems[satID][nowIdx] + ResArr2[key].FIDOtems[satID][nowIdx])/1e6, ':', linewidth=lw, color=co, zorder=zord)
                #     if OSP.isSat or plotn:
    ...
    print(mindate)
    print(maxdate)
    # for key in ResArr2.keys():
    #     print("second pass")
    #     lab = None
    #     multiBF = False
    #     # |------------- Generic case colors --------------|
    #     lw, co, zord, lab = 1, 'DarkBlue', 2, [None]
    #     # |--------------- Seed case colors ---------------|
    #     if key == 0:
    ...
    #
    #         # |------------- Make sure this member is an impact --------------|
    #     if ResArr2[key].FIDOtimes[satID] is not None:
    #         # |------------- No date (generic time) mode --------------|
    #         if OSP.noDate:
    #             dates = ResArr2[key].FIDOtimes[satID]
    #         # |------------- Convert FIDO time to real dats --------------|
    #         else:
    #             base = datetime.datetime(yr, 1, 1, 0, 0)
    #             if not OSP.doANT:
    #                 dates = np.array([base + datetime.timedelta(days=(i - 1)) for i in ResArr2[key].FIDOtimes[satID]])
    #             else:
    #                 dates = np.array([base + datetime.timedelta(days=(i + DoY)) for i in ResArr2[key].FIDOtimes[satID]])
    #
    #         # |------------- Plot the flux rope --------------|
    #         if not ResArr2[key].sheathOnly[satID]:
    #             nowIdx = ResArr2[key].FIDO_FRidx[satID]
    #             if lab[0]:
    #                 if multiBF:
    #                     for iii in BFidxs:
    #                         axes[0].plot(dates[nowIdx], ResArr2[key].FIDOBs[satID][nowIdx], linewidth=lw, color=co,
    #                                      zorder=zord, label=satNames[iii])
    #                 else:
    #                     axes[0].plot(dates[nowIdx], ResArr2[key].FIDOBs[satID][nowIdx], linewidth=lw, color=co,
    #                                  zorder=zord, label=lab)
    #
    #             else:
    #                 axes[0].plot(dates[nowIdx], ResArr2[key].FIDOBs[satID][nowIdx], linewidth=lw, color=co, zorder=zord)
    #             axes[1].plot(dates[nowIdx], ResArr2[key].FIDOBxs[satID][nowIdx], linewidth=lw, color=co, zorder=zord)
    #             axes[2].plot(dates[nowIdx], ResArr2[key].FIDOBys[satID][nowIdx], linewidth=lw, color=co, zorder=zord)
    #             axes[3].plot(dates[nowIdx], ResArr2[key].FIDOBzs[satID][nowIdx], linewidth=lw, color=co, zorder=zord)
    #             axes[4].plot(dates[nowIdx], ResArr2[key].FIDOvs[satID][nowIdx], linewidth=lw, color=co, zorder=zord)
    #             axes[5].plot(dates[nowIdx], ResArr2[key].FIDOtems[satID][nowIdx] / 1e6, linewidth=lw, color=co,
    #                          zorder=zord)
    #             # Option to plot either n or Kp
    #             if OSP.isSat or plotn:
    #                 axes[6].plot(dates[nowIdx], ResArr2[key].FIDOns[satID][nowIdx], linewidth=lw, color=co, zorder=zord)
    #             else:
    #                 axes[6].plot(dates[nowIdx], ResArr2[key].FIDOKps[satID][nowIdx], linewidth=lw, color=co, zorder=zord)
    #
    #             # |------------- Establish min/max date if not set --------------|
    #             if mindate is None:
    #                 mindate = dates[nowIdx[0]]
    ...
    #         # Have to make sure to connect the last sheath
    #         # point to the front of the FR
    #         if len(ResArr2[key].FIDO_shidx[satID]) != 0:
    #             nowIdx = ResArr2[key].FIDO_shidx[satID]
    #             if not ResArr2[key].sheathOnly[satID]:
    #                 nowIdx = np.append(nowIdx, ResArr2[key].FIDO_FRidx[satID][0])
    #             axes[0].plot(dates[nowIdx], ResArr2[key].FIDOBs[satID][nowIdx], '--', linewidth=lw, color=co,
    #                          zorder=zord)
    #             axes[1].plot(dates[nowIdx], ResArr2[key].FIDOBxs[satID][nowIdx], '--', linewidth=lw, color=co,
    #                          zorder=zord)
    #             axes[2].plot(dates[nowIdx], ResArr2[key].FIDOBys[satID][nowIdx], '--', linewidth=lw, color=co,
    #                          zorder=zord)
    #             axes[3].plot(dates[nowIdx], ResArr2[key].FIDOBzs[satID][nowIdx], '--', linewidth=lw, color=co,
    #                          zorder=zord)
    #             axes[4].plot(dates[nowIdx], ResArr2[key].FIDOvs[satID][nowIdx], '--', linewidth=lw, color=co,
    #                          zorder=zord)
    #             axes[5].plot(dates[nowIdx], ResArr2[key].FIDOtems[satID][nowIdx] / 1e6, '--', linewidth=lw, color=co,
    #                          zorder=zord)
    #             if OSP.isSat or plotn:
    #                 axes[6].plot(dates[nowIdx], ResArr2[key].FIDOns[satID][nowIdx], '--', linewidth=lw, color=co,
    #                              zorder=zord)
    #             else:
    #                 axes[6].plot(dates[nowIdx], ResArr2[key].FIDOKps[satID][nowIdx], '--', linewidth=lw, color=co,
    #                              zorder=zord)
    #
    #             # |------------- Establish min/max date if not set --------------|
    #             if mindate is None:
    ...
    #             # |------------- Print the arrival times of the sheath  --------------|
    #             if not silent:
    #                 print('ATs: ', dates[nowIdx[0]].strftime('%Y-%m-%dT%H:%M'),
    #                       dates[ResArr2[key].FIDO_FRidx[satID][0]].strftime('%Y-%m-%dT%H:%M'),
    #                       dates[ResArr2[key].FIDO_FRidx[satID][-1]].strftime('%Y-%m-%dT%H:%M'))
    #
    #         # |------------- No sheath, print the ATs of the FR --------------|
    #         else:
    #             if not silent:
    #                 print('ATs: ', dates[ResArr2[key].FIDO_FRidx[satID][0]].strftime('%Y-%m-%dT%H:%M'),
    #                       dates[ResArr2[key].FIDO_FRidx[satID][-1]].strftime('%Y-%m-%dT%H:%M'))
    #
    #         # |------------- Add padding of ambient SW surrounding the event --------------|
    #         if len(ResArr2[key].FIDO_SWidx[satID]) > 0:
    #             # |------------- Get current front/back --------------|
    #             if len(ResArr2[key].FIDO_shidx[satID]) != 0:
    #                 if not ResArr2[key].sheathOnly[satID]:
    #                     frontEnd, backStart = dates[ResArr2[key].FIDO_shidx[satID][0]], dates[
    #                         ResArr2[key].FIDO_FRidx[satID][-1]]
    #                 else:
    #                     frontEnd, backStart = dates[ResArr2[key].FIDO_shidx[satID][0]], dates[
    #                         ResArr2[key].FIDO_shidx[satID][-1]]
    #             else:
    #                 frontEnd, backStart = dates[ResArr2[key].FIDO_FRidx[satID][0]], dates[
    #                     ResArr2[key].FIDO_FRidx[satID][-1]]
    #
    #             # |------------- Add extra padding around event --------------|
    #             if OSP.noDate:
    #                 frontStart, backEnd = frontEnd - SWpadF, backStart + SWpadB
    #             else:
    #                 frontStart, backEnd = frontEnd - datetime.timedelta(hours=SWpadF), backStart + datetime.timedelta(
    #                     hours=SWpadB)
    #
    #             # |------------- Find corresponding indices --------------|
    #             frontIdx = np.where((dates >= frontStart) & (dates <= frontEnd))[0]
    ...
    for name in satNames:
        goodOrder.append(name)
        handles, labels = reorderLegend(axes[0], goodOrder)
        fig.legend(handles, labels, loc='upper center', fancybox=True, fontsize=13, labelspacing=0.4, handletextpad=0.4,
                   framealpha=0.5, ncol=(len(labels)))
